...
namespace Messenger
{
    public partial class responder : System.Web.UI.Page
    {
                ...
                switch (Request.QueryString["Action"])
                {
                    ...
                    case "signup":
                        ...
                        else
                        {
                            ...
                            sendMail(Request.QueryString["Email"], activationCode); // Send Activation Email

                            /***
                             * Insert Data Into Databse
                             */
                            sqlConnection.Close();  // Close Connection
                            sqlCommand = new SqlCommand("INSERT INTO Members (Members_FullName, Members_UserName, Members_Email, Members_Password, Members_ActivationCode) VALUES (@fullname, @username, @email, @password, @activationCode)", sqlConnection);    // Initialize command
        ...
        protected string getRandomChar(char firstChar)
        {
            ...
            return firstChar + chars[new Random().Next(0, chars.Length)].ToString() + chars[new Random().Next(0, chars.Length)].ToString() + chars[new Random().Next(0, chars.Length)].ToString();
        }

        /***
         * Sends the activation email
         */
        protected void sendMail(string emailAddress, string activationCode)
        {
            /***
